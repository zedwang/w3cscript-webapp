 ...
 *
 * Put the content of the modal.html inside of an `<ion-modal.html-view>` element.
 *
 * **Notes:**
 * - A modal.html will broadcast 'modal.html.shown', 'modal.html.hidden', and 'modal.html.removed' events from its originating
 * scope, passing in itself as an event argument. Both the modal.html.removed and modal.html.hidden events are
 * called when the modal.html is removed.
 *
 * - This example assumes your modal.html is in your main index file or another template file. If it is in its own
 * template file, remove the script tags and call it by file name.
 * 
 * @usage
 * ```html
 * <script id="my-modal.html.html" type="text/ng-template">
 *   <ion-modal.html-view>
 *     <ion-header-bar>
 ...
 *   $ionicModal.fromTemplateUrl('my-modal.html.html', {
 *     scope: $scope,
 *     animation: 'slide-in-up'
 *   }).then(function(modal.html) {
 *     $scope.modal.html = modal.html;
 *   });
 *   $scope.openModal = function() {
 *     $scope.modal.html.show();
 *   };
 *   $scope.closeModal = function() {
 *     $scope.modal.html.hide();
 *   };
 *   //Cleanup the modal.html when we're done with it!
 *   $scope.$on('$destroy', function() {
 *     $scope.modal.html.remove();
 *   });
 *   // Execute action on hide modal.html
 *   $scope.$on('modal.html.hidden', function() {
 *     // Execute action
 *   });
 *   // Execute action on remove modal.html
 *   $scope.$on('modal.html.removed', function() {
 *     // Execute action
 *   });
 * });
 * ```
...
function($rootScope, $ionicBody, $compile, $timeout, $ionicPlatform, $ionicTemplateLoader, $q, $log) {
   ...
   *
   * Be sure to call [remove()](#remove) when you are done with each modal.html
   * to clean it up and avoid memory leaks.
   *
   * Note: a modal.html will broadcast 'modal.html.shown', 'modal.html.hidden', and 'modal.html.removed' events from its originating
   * scope, passing in itself as an event argument. Note: both modal.html.removed and modal.html.hidden are
   * called when the modal.html is removed.
   */
  var ModalView = ionic.views.Modal.inherit({
     ...
     * @description Creates a new modal.html controller instance.
     ...
     *    the modal.html when shown.  Default: false.
     *  - `{boolean=}` `backdropClickToClose` Whether to close the modal.html on clicking the backdrop.
     *    Default: true.
     *  - `{boolean=}` `hardwareBackButtonClose` Whether the modal.html can be closed using the hardware
     ...
     * @description Show this modal.html instance.
     * @returns {promise} A promise which is resolved when the modal.html is finished animating in.
     */
     ...
     * @description Hide this modal.html instance.
     * @returns {promise} A promise which is resolved when the modal.html is finished animating out.
     */
    hide: function() {
      var self = this;
      var modalEl = jqLite(self.modalEl);
     ...
     * @description Remove this modal.html instance from the DOM and clean up.
     * @returns {promise} A promise which is resolved when the modal.html is finished animating out.
     */
    remove: function() {
      var self = this;
     ...
     * @returns boolean Whether this modal.html is currently shown.
     */
    isShown: function() {
  ...
  var createModal = function(templateString, options) {
    // Create a new scope for the modal.html
  ...
  return {
     ...
     * @param {string} templateString The template string to use as the modal.html's
     * content.
  ...
  return {
    $get: ['$q', '$rootScope', function($q, $rootScope) {
      var self = {
         ...
         * or close a modal.html which may be open.
         *
...
.directive('ionModal', [function() {
  return {
    ...
    replace: true,
    controller: [function(){}],
    template: '<div class="modal.html-backdrop">' +
                '<div class="modal.html-wrapper" ng-transclude></div>' +
                '</div>'
  };
}]);
  ...
  return {
    ...
    compile: function(tElement, tAttrs, transclude) {
      ...
      return function link($scope, $element, $attr, ctrls) {
        ...
        //Don't use the ionView if we're inside a modal.html or there's no navbar
